...
--
-- 目的: LINE⇔Shopify アカウント連携を顧客自身が完了した瞬間に ¥300 OFF の
--   Shopify 実クーポンを 1 枚発行する (= LINE 到達可能な連携済み契約を動かす連携インセンティブ)。
--   対象経路 = sub-link redeem (App Proxy / magic-link) と
--   email OTP verify の顧客対話 2 系統。cron 逆引き / admin import は対象外。
--   ⚠️ 額は 2026-08-11 Katsu 決定で ¥500 → ¥300。**表示・発行の正は台帳のこの列**であって
--   列の DEFAULT でも定数でもない (INSERT は常に値を明示 bind する)。DEFAULT は
--   「値を渡さない経路が万一できたとき」の保険なので、定数と揃えておく。
--
