...
const NAV_STATE_JS: string = [
  ...
  'var navEntry = null;',
  '// 顧客が自分でスクロールしたか。 復元は操作を奪わない (立った瞬間に打ち直しをやめる)。',
  ...
  '  var tries = 0;',
  '  function apply() {',
  '    // 🚨 キャンセル判定は initNavState で**初期化直後から**張った navUserScrolled を見る。',
  '    //    ここで listener を張り始めると、 loading が剥がれてから復元が走るまでの間に',
  '    //    顧客がした操作を拾えない (採点ループ P2)。',
  '    if (navUserScrolled) return;',
  '    try {',
  ...
  '  } catch (e) { /* ignore */ }',
  '  try {',
  '    var onUser = function () { navUserScrolled = true; };',
