...
const REORDER_GUARD_JS: string = [
  ...
  '  // 🚨 閉じたら保留中の購入を必ず捨てる (採点ループ HIGH)。',
  '  //   残すと、次に別経路で開いた確認シートの「はい」がこの購入を横取りし、',
  '  //   顧客が意図した再注文は無音で消え、選んでいない商品のカートが開く。',
  '  if (typeof shopClearPendingBuy === "function") shopClearPendingBuy();',
  '}',
  '',
  ...
  '  closeSubDupConfirm();',
  '  // Shop v2 グリッドからの購入が待っていればそちらを続行する (gate off では未定義)',
  '  if (typeof shopBuyAckProceed === "function" && shopBuyAckProceed()) return;',
  '  rosAckSubDup = true;',
