    ...
    <div id="section-account" class="section space-y-4">
      ...
      <div class="card p-4">
        ...
        <!-- 🚨 アイコンは絵文字の代用でなく **公式マーク** を使う (2026-08-31 Katsu 指示)。
             ...
             ガイドライン上の想定内。🚨 パスは**手で書かない** — 記憶で書くと
             「それらしいが実物と違う」形になる (2026-08-31 に Instagram で実際に起きた)。
             Simple Icons の公式配布データ (CC0) をそのまま埋め込むこと。
  ...
  try {
    ...
    if (res.data && res.data.faqs && res.data.faqs.length > 0) {
      ...
      items = [
        ...
        { question: '定期購入はできますか？', answer: 'はい。商品ページから定期便をお選びいただけます（2回目以降5%OFF）。スキップ・お届け日の変更・解約は、マイページ（naturism-diet.com/account）の「定期購買一覧」から24時間お手続きいただけます。なお「Shop」タブの「定期お届けリマインダー」は、お好みのサイクルで再注文のお知らせを受け取る機能で、定期便とは別のものです。', category: 'subscription' },
