...
/**
 * POST /api/conductor/message (Phase 5γ-4)
 * body: { prompt: string }
 * 200: { success: true, data: { template, messageContent, messageType, altText, warnings, provider, model } }
 * 400/502/503/504/500: error code mapping
 */
conductor.post('/api/conductor/message', async (c) => {
  let body: { prompt?: unknown };
  try {
    body = await c.req.json<{ prompt?: unknown }>();
  } catch {
  ...
  try {
    ...
    if (err instanceof MessageConductorError) {
      return c.json(
        { success: false, error: err.message, code: err.code },
        mapErrorCodeToStatus(err.code),
      );
    }
    console.error('POST /api/conductor/message error:', err);
    return c.json(
      { success: false, error: 'Internal server error' },
      500,
    );
