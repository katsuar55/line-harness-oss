...
vi.mock('../services/form-conductor.js', async (importOriginal) => {
  const orig = (await importOriginal()) as typeof import('../services/form-conductor.js');
  return {
    ...orig,
    generateFormFromPrompt: vi.fn(),
  };
});
...
// ============================================================
// Phase 5γ-3: form route
// ============================================================

const VALID_FORM_RESULT = {
  form: {
    name: 'test form',
    description: null,
    fields: [{ name: 'q1', label: '質問', type: 'text' }],
    onSubmitTagId: null,
...
describe('POST /api/conductor/form — authentication', () => {
  let app: ReturnType<typeof createTestApp>;
  let env: Env['Bindings'];
...
describe('POST /api/conductor/form — body validation', () => {
  let app: ReturnType<typeof createTestApp>;
  let env: Env['Bindings'];
...
describe('POST /api/conductor/form — service error → HTTP mapping', () => {
  let app: ReturnType<typeof createTestApp>;
  let env: Env['Bindings'];
  const mockedGenerateForm = generateFormFromPrompt as ReturnType<typeof vi.fn>;
  ...
  it('returns 200 on success', async () => {
    mockedGenerateForm.mockResolvedValueOnce(VALID_FORM_RESULT);
    const res = await app.request(
      '/api/conductor/form',
      {
        method: 'POST',
        headers: authHeaders(),
        body: JSON.stringify({ prompt: '商品アンケート' }),
      },
      env,
    );
    expect(res.status).toBe(200);
    const body = (await res.json()) as { success: boolean; data: unknown };
    expect(body.success).toBe(true);
    expect(body.data).toEqual(VALID_FORM_RESULT);
  });

  it('maps api_key_missing to 503', async () => {
    mockedGenerateForm.mockRejectedValueOnce(
      new FormConductorError('no provider', 'api_key_missing'),
    );
    const res = await app.request(
      '/api/conductor/form',
      {
  ...
  it('maps schema_validation_failed to 502', async () => {
    mockedGenerateForm.mockRejectedValueOnce(
      new FormConductorError('bad', 'schema_validation_failed'),
    );
    const res = await app.request(
      '/api/conductor/form',
      {
  ...
  it('maps timeout to 504', async () => {
    mockedGenerateForm.mockRejectedValueOnce(
      new FormConductorError('timed out', 'timeout'),
    );
    const res = await app.request(
      '/api/conductor/form',
      {
  ...
  it('maps unknown error to 500', async () => {
    mockedGenerateForm.mockRejectedValueOnce(new Error('unexpected'));
    const res = await app.request(
      '/api/conductor/form',
      {
        method: 'POST',
