...
4. **薬機法遵守（最重要）**: 「痩せる」「治る」「効く」「効果がある」「改善する」「向上する」等の効能効果を断定する表現は絶対に使わない。機能性表示食品の届出表示を引用する場合も「届出表示に基づき〜をサポートします」のように記載し、「〜の効果があります」とは言い換えない。常に「〜をサポート」「〜のために設計」「〜を目指す方に」等の表現を使う
