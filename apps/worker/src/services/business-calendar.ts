 ...
 *
 * ## 土曜は定休 (2026-08-08 Katsu 確定)
 * 一度「休みは日曜と祝日」= 土曜も営業、との指示があったが、**commit 5838fa6
 * (2026-06-15「運用系ファクトを公式ポリシー準拠に修正」) が逆方向に確定済み**であること
 * (旧「日祝休み」→ 新「土日祝・年末年始を除く」。テストにも「旧『日祝休み』=
 * **土曜営業の誤解** を土日祝に修正」と明記) と、ai-response / ai-message-builder /
 * faq-context / contact-email-page / seed-naturism-faq-v3.sql の **5 箇所が
 * 「土日祝休み」で顧客に案内している**事実を提示して確認したところ、
 * **「土曜は休み」で確定**した。よって定休 = 日曜 + 土曜 + 祝日。
 *
 * ⚠️ 将来この判断を覆すときは `SATURDAY_IS_BUSINESS_DAY` を true にするだけでは足りない —
 * **上記 5 箇所の顧客向け文言も同時に更新すること** (片方だけ変えると案内と実態が食い違い、
 * 出社しない日を約束して §4-2 の謝罪 push が構造的に量産される)。
 *
 * ## 受理時刻を見ない設計
...
/**
 * 土曜を営業日として扱うか。**false = 定休** (2026-08-08 Katsu 確定。顧客向け案内
 * 「受付: 平日10:00〜17:00 (土日祝・年末年始を除く)」5 箇所と一致する)。
 * 覆すときは顧客向け文言 5 箇所も同時に更新すること (ヘッダのコメント参照)。
 */
