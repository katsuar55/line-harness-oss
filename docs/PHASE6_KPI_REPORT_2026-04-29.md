...
| `users.phone` が NULL でない | **0** | 同様に致命的: `shopify.ts` のフォールバックで phone マッチングも試みる (`users.phone = shopify_customers.phone`) が、こちらも空 |
| `shopify_customers.email` が NULL でない | 291 | Shopify 側はメール持っている |
| `users` × `shopify_customers` の email 一致 | **0** | 上記の通り (left = NULL) |

**結論**: Phase 6 PR-2 (orders/create → enroll) は、**仮に 7 日待って 100 件の orders/create が来ても 1 件も enroll できない**。

**マッチングコードパス全容** (`shopify.ts:130-168`):
1. 注文の email で `users.email` を引き当て → `friends.user_id` で friend 取得 (現状 0 マッチ)
2. 1 で見つからない場合、`users.phone` を引き当て (現状 0 マッチ)
3. どちらも見つからない場合 → friendId = null で enroll はスキップ

**真の原因**: LIFF Login 時 (`liff.ts:303-308`) に保存しているのは `email` と `displayName` のみで、`phone` は最初から保存されない。さらに 1 件しか居ない user record の email も NULL のままになっている = LIFF Email Scope の id_token 取得経路が動いていない。`users.email` を埋める仕組み (LINE Console での email scope 申請承認 + Shopify webhook での back-fill) が先に必要。
...
- [x] /liff/cart 500 hotfix v1 (notFound 再帰バグ + cart redirect → reorder) — commit `322bb46`
- [x] /liff/cart redirect v2 修正: `/liff/reorder` (= reminder 編集 SPA) ではなくメニューハブ `/liff/portal` に変更 + クエリ文字列引き継ぎ + 追加テスト 3 件 (2026-04-29 自己レビュー)
- [x] 本レポート作成

### 適用済 migration 039 の動作確認 (2026-04-29 07:15 JST 時点)

`SELECT job_name, status, COUNT(*) FROM cron_run_logs GROUP BY job_name, status` 結果:
- `subscription-reminder` / `success` / **17 行**
- 2026-04-29 06:05 JST から 5 分間隔で書き込み (= 14 ティック + 数件の追加 = 整合)
- **他の cron job (BAN-monitor / token-refresh / step-delivery / scheduled-broadcasts 等) は cron_run_logs に書いていない**

→ 別途 `cron-monitor.DEFAULT_RULES` への job 追加 + 各サービスへの heartbeat 書き込み呼び出しを Phase 7 として整備する必要あり。
