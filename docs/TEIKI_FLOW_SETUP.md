...
**実測を受け始める前に済ませること** — 未適用のまま実測が入ると、その後お客様がスキップしても
決済日が動かず、**スキップ済みの顧客に 1 周期古い決済リマインドを送る**ことになる。
...
- 実測値は**新しく見える実注文が来ると `derived` に戻る**。
  ...
  差し戻し先が窓 (`[3,7]` 日前) の外なら無送信で、
  **次の Flow 発火 (7日前通知 / スキップ / お届け日変更) で自然に回復する**
  (直近注文 + 周期が偶然窓に入ればリマインドは出るので、誤送信ゼロではない)。
  「実測日以降の注文だけが derived に戻す」という保護を検討したが棄却した —
  実測を保持した行は後続のスキップ先送りを一切反映しないため、
  **スキップ済みと分かっている顧客へ確実に古い決済日を送る**経路ができる。
  こちらの誤送信は偶然の一致に限られる、という非対称性で判断した
