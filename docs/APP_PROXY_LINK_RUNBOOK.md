...
### 2. ✅ 否定テスト — **2026-07-29 実施済み・合格**

**結論: Shopify は訪問者が書き足した `logged_in_customer_id` を自分の値で上書きする。**
したがってパラメータ注入は成立せず、この identity 設計で問題ない。

実測 (gate を一時 ON にして storefront から直接観測、架空の番号を使用):

| リクエスト | 結果 |
|---|---|
| 対照 (パラメータなし) | 200 「ログインしてLINEと連携」 |
| `?logged_in_customer_id=999999999999` | 200 「ログインしてLINEと連携」 |
| 同名パラメータを 2 つ (`=111&=222`) | 200 「ログインしてLINEと連携」 |
| 実在しうる形の番号 | 200 「ログインしてLINEと連携」 |
| 余計な param 併用 (`&foo=bar`) | 200 「ログインしてLINEと連携」 |
| サブパス経由 | 200 「ログインしてLINEと連携」 |

全パターンで **`login_required`** (= worker に届いた `logged_in_customer_id` は空)。
署名検証自体は通っている (通らなければ 404) ので、Shopify が値を差し替えたことが確定する。
危険ケースなら架空番号は当方 DB に無いため「もう少しお待ちください」(sync_pending) が出るはずで、
出方が明確に分かれる設計にしてある。テスト後は gate を `false` に戻し、404 復帰を確認済み。

> 再実施が必要になった場合 (Shopify の仕様変更を疑うとき等) は、以下の「実施方法」を使う。
