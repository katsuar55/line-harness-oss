...
## 公開ルート × オブジェクトストレージのルール (絶対遵守 — 再発防止)

2026-08-16 に **D1 日次バックアップ (顧客 PII を含む DB 全体) が無認証で公開ダウンロード可能**
だったことが発覚 (本番実測で 206 + 実 SQL)。原因は「認証 skip-list に載った公開ルートが、
外部から来た文字列をそのまま R2 のキーとして引いていた」こと。バケットを画像と
バックアップで共用していたため、画像配信ルートがバックアップの配信ルートにもなっていた。

### 禁止パターン

| パターン | 理由 |
|---|---|
| `authMiddleware` の skip-list に載ったルートで、リクエスト由来の文字列を **無検証で** `R2.get/head/delete` や `KV.get` に渡す | そのバインディングの**バケット/名前空間全体**が無認証で読める。`:key` が 1 セグメントに見えても、Hono は `%2F` をデコードして渡すのでスラッシュ入りキーが通る |
| 機微データと公開データを**同じバケット**に置き、プレフィックスだけで分ける | 分離がコード 1 箇所のガードに依存する。ガードを消した/広げた瞬間に全公開に戻る |
| 「`backups/` を弾く」という **deny-list** で塞ぐ | 将来そのバケットに別の機微プレフィックスを足した人が無言で漏らす。列挙すべきは「公開してよいもの」 |
| 拒否を **403** で返す | 「弾かれた」と「存在しない」が区別でき、鍵の存在確認オラクルになる |
| ルートの match を `:key{.+}` 等へ**広げる**ときに allowlist を確認しない | 広げた瞬間に多段キーがハンドラへ到達する。allowlist だけが防壁 |

### 推奨パターン

| やりたいこと | 正しい方法 |
|---|---|
| 公開ルートでオブジェクトを引く | **allowlist** で key の形を検証してから引く (`isPubliclyServableImageKey` = `routes/images.ts`)。スラッシュを含む名前空間は明示列挙し、セグメントに `/` を含めない |
| 拒否の返し方 | 「存在しない」と**バイト単位で同一**の応答にする |
| ガードが効いていることの検証 | ステータスコードだけを見ない。**R2 を一度も呼んでいないこと** (`expect(bucket.head).not.toHaveBeenCalled()`) を観測点にする。ステータスだけだと「読んでから潰す」実装で緑になる |
| 新しい機微データの置き場所 | 公開バインディングと**別のバケット**に置く (defense-in-depth)。同居させるなら allowlist に載せないだけでは不十分と考える |

### 自己点検チェックリスト (公開ルートを追加/変更する前)

- [ ] このパスは `middleware/auth.ts` の skip-list に載るか? 載るなら**無認証で誰でも叩ける**と考えたか?
- [ ] 外部由来の文字列がストレージのキー・パス・識別子として使われていないか?
- [ ] 使うなら **allowlist** で形を検証しているか? (deny-list になっていないか)
- [ ] そのバインディングに**機微データが同居**していないか? (バケットの中身を実際に確認したか)
- [ ] 拒否応答は「存在しない」と区別できないか?
- [ ] テストの観測点は「ストレージに触れていないこと」になっているか?

### 違反時の必須アクション

新パターンで公開ルートからデータが漏れた場合、本ファイルの「禁止パターン」表に追記してから
次の作業に移る。あわせて **露出期間と対象データを実測で特定**し (workflow の run 履歴・
バケットの lifecycle rule 等)、消せない既成事実として Katsu に報告する。
...
2026-08-22 に「deploy 済み・curl では新マーカーが出るのに、オーナー実機の LINE ミニアプリは
何も変わらない」事故が発生した (#270 の視覚改修)。原因は **LIFF ページ 7 本と管理画面の HTML が、
リポジトリ開設以来 `Cache-Control` / `ETag` / `Last-Modified` を 1 つも返していなかった**こと。
妥当性情報がゼロの HTML を受け取った LINE の in-app WebView はキャッシュ挙動が完全に実装依存になり、
古い HTML を配り続ける。**post-deploy-check も本番 curl も「サーバは正しい」としか言わないため、
この層の事故は実機からの報告でしか発覚しない。**

### 禁止パターン

| パターン | 理由 |
|---|---|
| 顧客が見る HTML を `c.html(...)` 素通しで返す (Cache-Control なし) | validator ゼロ = キャッシュ挙動が WebView 実装依存。deploy しても実機に届かない。**サーバ側の実測 (curl/post-deploy-check) は全部 green のまま**なので、壊れていることに気付けない |
| 「本番 curl で新マーカーが出た」= 「実機に反映された」と報告する | 別の層 (クライアントキャッシュ) を一切観測していない。curl は cold client であって実機ではない |
| キャッシュヘッダをハンドラごとに個別に付ける | 新しい LIFF ページを足した人が忘れた瞬間に同じ事故が再発する。単一の chokepoint に置く |
| キャッシュ対象の判定を**パスの denylist** で書く | 新ページ追加で漏れる。`Content-Type` で判定すれば画像/JSON は構造的に対象外になる |
| ヘッダ修正だけして「直った」と報告する | **no-store は既に端末にあるキャッシュを消さない**。修正後も 1 回は強制再取得 (LINE のキャッシュ削除 / 外部ブラウザで確認) が要る。ユーザーにそれを伝えるまでが修正 |

### 推奨パターン

| やりたいこと | 正しい方法 |
|---|---|
| 顧客向け HTML を常に最新で配る | `middleware/html-cache.ts` の `htmlNoStoreMiddleware` (index.ts に `app.use('*', ...)` で 1 箇所)。`Content-Type: text/html` のときだけ `no-store` を付け、明示済みの `Cache-Control` は上書きしない |
| 画像等のキャッシュは残す | Content-Type で自動的に対象外 (`brand-logo.png` の `immutable` は無傷)。テストで退行を固定する |
| ETag 方式を検討する | portal は 343KB (gzip 100KB)。毎リクエストのハッシュ計算は Workers の CPU 10ms 制限を圧迫するため**採らない**。correctness を優先して no-store |
| 配線が消えたことを検出する | テストがミドルウェアを**手配線**すると `index.ts` から `app.use` を消しても green のまま。**本番の default export (`worker.fetch`) を実際に叩く**テストを 1 件必ず置く |
| 実機の鮮度を疑ったとき | ①本番 curl でマーカー確認 (サーバ層) ②レスポンスヘッダに `Cache-Control` があるか (配信層) ③実機で外部ブラウザ / キャッシュ削除後に再確認 (クライアント層)。**3 層を分けて観測する** |

### 自己点検チェックリスト (顧客が見る HTML を追加/変更する前)

- [ ] そのルートは `Content-Type: text/html` を返すか? 返すなら chokepoint の middleware を通るか?
- [ ] キャッシュ判定を Content-Type でなくパスで書いていないか?
- [ ] 既に `Cache-Control` を明示しているルートを上書きしていないか? (画像の immutable を壊さないか)
- [ ] 配線 (`app.use`) が消えたら落ちるテストがあるか? (手配線テストだけになっていないか)
- [ ] 出荷報告に「実機で確認する手順」を含めたか? (no-store は既存キャッシュを消さない)

### 違反時の必須アクション

新パターンで「サーバは新しいのに実機が古い」が発生したら、本ファイルの「禁止パターン」表に
追記してから次の作業に移る。あわせて**どの層で止まっていたか** (サーバ / 配信ヘッダ /
クライアントキャッシュ) を実測で特定して Katsu に報告する。
